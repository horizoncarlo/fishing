...
function initLand() {
    ...
    pierDiv.style.top = getRandomInt(PIER_HEIGHT*1.3, getDocumentHeight()-PIER_HEIGHT*1.3) + 'px';
    ...
    menu.id = 'shackMenu';
    menu.className = 'shackMenu';
    menu.style.display = 'none';
    addChild(menu);
    shack.addEventListener('contextmenu', function(e) {
        e.preventDefault();
        ...
        if (shackMenu) {
            ...
            // Add some menu items, a bit messily
            shackMenu.innerHTML = '';
            if (birdInterval) {
                shackMenu.innerHTML += '<button onclick="closeShackMenu(); removeBirds();">&check; Birds</button>';
            }
            else {
                shackMenu.innerHTML += '<button onclick="closeShackMenu(); initBirds();">&cross; Birds</button>';
            }
            
            if (document.getElementById('crab')) {
                shackMenu.innerHTML += '<button onclick="closeShackMenu(); removeCrab();">&check; Crab</button>';
            }
            else {
                shackMenu.innerHTML += '<button onclick="closeShackMenu(); initCrab();">&cross; Crab</button>';
            }
            
            if (document.getElementById('sun')) {
                shackMenu.innerHTML += '<button onclick="closeShackMenu(); removeSun();">&check; Sun</button>';
            }
            else {
            ...
            if (difficulty.ALLOW_SOUND) {
                if (typeof beep !== 'undefined' && player.soundOn) {
                    shackMenu.innerHTML += '<button onclick="closeShackMenu(); removeSound();">&check; Sound</button>';    
                }
                else {
                    shackMenu.innerHTML += '<button onclick="closeShackMenu(); initSound();">&cross; Sound</button>';
                }
            }
            ...
            shackMenu.innerHTML += '<center><button onclick="closeShackMenu();" style="width: auto;">Close</button></center>';
        }
    });
...
function closeShackMenu() {
    if (document.getElementById('shackMenu')) {
        document.getElementById('shackMenu').style.display = 'none';
    }
}
...
function initLandObjects() {
        ...
        if (Math.random() >= 0.2 || (tonsLittle && Math.random() >= 0.12)) {
            if (Math.random() >= 0.15 || wallCount >= 5) {
                if (!noShells && Math.random() >= 0.42) {
                    ...
                    currentBackground.src = './images/terrain/plant' + getRandomInt(1, 7) + '.png';
                    currentBackground.style.zIndex = 3;
                }
                else {
                    currentBackground.src = './images/terrain/grass' + getRandomInt(1, 7) + '.png';
                }
            }
            else if (!noWalls) {
    ...
    if (Math.random() > 0.7) {
        var log = applyLandObject('./images/terrain/log.png');
        if (getRandomBoolean()) {
            flipObject(log);
        }
    }
    ...
    if (Math.random() > 0.75) {
        var grass = applyLandObject('./images/terrain/scattered_grass.png', 1, true);
        // Don't spill into the water
        if (parseInt(grass.style.left)+175 >= LAND_WIDTH) {
            grass.style.left = parseInt(grass.style.left) - getRandomInt(175, 200);
        }
    }
    // Animated crab buddy
    if (Math.random() >= 0.3) {
...
function initCrab() {
    var crab = applyLandObject('./images/crab.gif', 2);
    crab.id = 'crab';
    
    // Silly color change when clicked
    crab.style.cursor = 'grabbing';
    crab.addEventListener('click', function() {
        var shift = getRandomInt(0, 360);
        crab.style.filter = 'hue-rotate(' + shift + 'deg)';
        
        // Have a staggeringly low chance for the crab to give up bait
        if (shift >= 350 && Math.random() <= 0.4) {
            var baitPlus = getRandomInt(1, 3);
            player.bait += baitPlus;
            updateScoreboard();
            
            showLongMessage('Lucky Crab gives ' + baitPlus + ' bait!', crab);
        }
    }, true);
    ...
    crabInterval = setInterval(function() {
        // Scurry around nicely. Decide a distance for X and Y and also determine if it's up or down
        var moveX = getRandomInt(50, 300);
        var moveY = getRandomInt(50, 300);
        moveX *= getRandomBoolean() ? 1 : -1;
        moveY *= getRandomBoolean() ? 1 : -1;
        
        // Check if our transform will take us off the page, in which case adjust it
        var crabRect = crab.getBoundingClientRect();
        var targetLoc = crabRect.y + moveY;
        if (targetLoc < 0) { // Bound by top of page
            moveY = (crabRect.y * -1) + getRandomInt(20, 30);
        }
        else if (targetLoc > getDocumentHeight()) { // Bound by bottom of page
            moveY = getDocumentHeight() - crabRect.y - getRandomInt(20, 30);
        }
        targetLoc = crabRect.x + moveX;
        if (targetLoc < 0) { // Left of the page
            moveX = moveX + Math.abs(targetLoc) + getRandomInt(0, 10);
        }
        else if (targetLoc > getDocumentWidth()) { // Right of the page
            moveX = getDocumentWidth() - crabRect.x - getRandomInt(40, 50);
        }
        
        // If we're in the water we want to change our opacity to look like we're swimming, and opposite for on land
        if ((crabRect.x + moveX - 40) > LAND_WIDTH) {
            var crossAt = (crabRect.x + moveX) - LAND_WIDTH;
            var changeOpac = (crabRect.x + moveX) - crossAt;
            var percentAt = changeOpac / (crabRect.x + moveX);
            var timeoutTime = 10 * percentAt; // 10 number is based on transition animation below
            setTimeout(function() {
                crab.style.opacity = 0.6;
            }, timeoutTime * 1000);
        }
        else {
            // Just guesstimate our land transition
            setTimeout(function() {
                crab.style.opacity = 1;
            }, getRandomInt(4500, 5500));
        }
        
        crab.style.transition = 'transform 10s linear';
        crab.style.transform = 'translate(' + moveX + 'px, ' + moveY + 'px)';
        
        // Once our transform is done we want to update our position
        // Otherwise we have an outdated position for calculating going off the screen
        setTimeout(function() {
            crab.style.transition = 'none';
            crab.style.transform = 'none';
            crab.style.left = crabRect.x + moveX + 'px';
            crab.style.top = crabRect.y + moveY + 'px';
        }, 10500);
    }, 15000);
}
...
function removeCrab() {
    if (document.getElementById('crab')) {
        ...
        deleteChild(document.getElementById('crab'));
    }
}
...
function removeBirds() {
    ...
    if (document.getElementById('bird')) {
        deleteChild(document.getElementById('bird'));
    }
}
...
function initWaterObjects() {
    ...
    // Unique single objects
    var submerged = Math.random() > 0.65;
    var submergedTree = getRandomBoolean();
    var submergedBoth = Math.random() > 0.9;
    if (submerged && (submergedTree || submergedBoth)) {
    ...
    // Have a chance for submerged rocks, but lower the odds if we already have a tree
    if (submerged && (!submergedTree || submergedBoth)) {
        // Chance for a double rock
        var rockCount = Math.random() > 0.8 ? 2 : 1;
        for (var i = 0; i < rockCount; i++) {
            currentObject = applyWaterObject('./images/terrain/submerged_rock' + getRandomInt(1, 3) +'.png', 2);
            currentObject.addEventListener('click', clickToFish, true);
            if (getRandomBoolean()) {
                flipObject(currentObject);
            }
    ...
    if (Math.random() > 0.7) {
        currentObject = applyWaterObject('./images/terrain/log.png');
        currentObject.addEventListener('click', clickToFish, true);
        currentObject.style.opacity = 0.85;
        if (getRandomBoolean()) {
            flipObject(currentObject);
        }
    }
